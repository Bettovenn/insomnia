...
export const RequestPane: FC<Props> = ({
  ...
  const addTag = () => {
    if (newTag && (!activeRequest.tags || !activeRequest.tags.includes(newTag))) {
      const updatedTags = [...(activeRequest.tags || []), newTag];
      patchRequest(requestId, { tags: updatedTags });
      setNewTag('');
    }
  };

  const removeTag = (tagToRemove: string) => {
    const updatedTags = activeRequest.tags?.filter(tag => tag !== tagToRemove) || [];
    patchRequest(requestId, { tags: updatedTags });
  };
  ...
  return (
      <Pane type="request">
        ...
        <Tabs aria-label='Request pane tabs' className="flex-1 w-full h-full flex flex-col">
          ...
          {/* Nuevo TabPanel para Tags */}
          <TabPanel className='w-full flex-1 overflow-y-auto p-4' id='tags'>
            <div className="flex flex-col">
              <Heading className='text-xs font-bold uppercase text-[--hl] mb-4'>Module Tags</Heading>
              <div className="flex items-center gap-2 mb-4">
                <input
                    type="text"
                    value={newTag}
                    onChange={(e) => setNewTag(e.target.value)}
                    className="flex-1 bg-[--color-bg] border border-[--hl-md] p-2 rounded-sm focus:outline-none focus:border-[--hl]"
                ...
                />
                <Button
                    className="px-4 py-2 bg-[--color-bg] hover:bg-[--hl-xs] border border-[--hl-md] rounded-sm"
                    onPress={addTag}
                >
                  Add Tag
                </Button>
              ...
              {activeRequest.tags && activeRequest.tags.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {activeRequest.tags.map(tag => (
                        <div
                            key={tag}
                            className="flex items-center bg-[--hl-xs] hover:bg-[--hl-sm] rounded-full px-3 py-1"
                        >
                  ...
                  <div className="text-[--hl] italic text-center p-4">
                    No tags added yet. Add tags to organize your endpoints by module.
                  </div>
              )}
            </div>
          </TabPanel>
